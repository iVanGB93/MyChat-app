...
export type ImportedSticker = { id: string; name: string };
...
export function importedStickerUri(owner: number, sticker: ImportedSticker): string {
  ...
  return new File(directory(owner), `${sticker.id}.png`).uri;
}
...
export function importSticker(owner: number, uri: string, name: string): Promise<ImportedSticker[]> {
  const task = writes.then(async () => {
    ...
    validateStickerFile(await source.bytes());
    ...
    const normalized = await manipulateAsync(uri, Math.max(width, height) > 512 ? [{ resize: width >= height ? { width: 512 } : { height: 512 } }] : [], { format: SaveFormat.PNG });
    ...
    try {
      ...
      const dest = new File(dir, `${id}.png`);
      const created = !dest.exists;
      if (created) file.copy(dest);
      const updated = exists ? rows : [...rows, { id, name: name.slice(0, 80) || 'Imported sticker' }];
